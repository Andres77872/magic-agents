...
class Node(abc.ABC):
    ...
    def __init__(
        ...
        # Subclasses may consult the active registry from ``process()``. Keep
        # this defined for direct process-level execution; ``__call__`` replaces
        # it with the registry for each normal graph invocation.
        self._hooks = None
